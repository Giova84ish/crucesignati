...
export class CrucesignatiActorSheetMonster extends CrucesignatiActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["crucesignati", "sheet", "monster", "actor"],
      template: "systems/crucesignati/templates/actors/monster-sheet.html",
      width: 450,
      height: 560,
      resizable: true,
      tabs: [
   ...
   */
  async generateSave() {
    let choices = CONFIG.CRUCESIGNATI.monster_saves;

    let templateData = { choices: choices },
      dlg = await renderTemplate(
        "/systems/crucesignati/templates/actors/dialogs/monster-saves.html",
        templateData
      );
    //Create Dialog window
    new Dialog({
      title: game.i18n.localize("CRUCESIGNATI.dialog.generateSaves"),
      content: dlg,
      buttons: {
        ok: {
  ...
  async _chooseItemType(choices = ["weapon", "armor", "shield", "gear"]) {
    ...
    return new Promise((resolve) => {
      new Dialog({
        title: game.i18n.localize("CRUCESIGNATI.dialog.createItem"),
        content: dlg,
        buttons: {
          ok: {
            label: game.i18n.localize("CRUCESIGNATI.Ok"),
