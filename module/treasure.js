export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("crucesignati", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("crucesignati", "treasure");
    table.object.setFlag("crucesignati", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('CRUCESIGNATI.table.treasure.roll')}</button>`;
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/crucesignati/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/crucesignati/assets/coins.mp3"
  }
