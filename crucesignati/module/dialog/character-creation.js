...
export class OseCharacterCreator extends FormApplication {
  static get defaultOptions() {
    ...
    options.template =
      "systems/crucesignati/templates/actors/dialogs/character-creation.html";
    options.width = 235;
    return options;
  }
   ...
   */
  get title() {
    return `${this.object.name}: ${game.i18n.localize("CRUCESIGNATI.dialog.generator")}`;
  }
  ...
  rollScore(score, options = {}) {
    ...
    const label =
      score != "gold" ? game.i18n.localize(`CRUCESIGNATI.scores.${score}.long`) : "Gold";
    const rollParts = ["3d6"];
    const data = {
      roll: {
        type: "result",
    ...
    return OseDice.Roll({
      ...
      title: game.i18n.format("CRUCESIGNATI.dialog.generateScore", {
        score: label,
        count: this.counters[score],
      }),
    });
  ...
  async close(options) {
    ...
    const templateData = {
      ...
      scores: this.scores,
      title: game.i18n.localize("CRUCESIGNATI.dialog.generator"),
      stats: this.object.data.stats,
      gold: this.gold,
    };
    const content = await renderTemplate(
      "/systems/crucesignati/templates/chat/roll-creation.html",
      templateData
    );
    ChatMessage.create({
      content: content,
